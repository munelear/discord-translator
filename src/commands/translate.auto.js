...
module.exports = function (data) {
  ...
  if (data.cmd.from === "auto" || data.cmd.from.valid.length !== 1) {
    data.color = "error";
    data.text = ":warning:  Please specify a valid language to translate from.";
  ...
  const taskLoop = function () {
    data.task.for.forEach(
      (
        ...
        if (dest.startsWith("@") || (!dest.startsWith("<") && dest !== "me")) {
          data.task.invalid.push(dest);
          taskBuffer.reduce();
        }
      }
  ...
  const validateTask = function () {
    ...
    const forNames = data.cmd.for
      .join(",  ")
      .replace("me", `<@${data.message.author.id}>`);
